...
for t=1:length(testNames)
    ...
    for testSubjID=1:length(filenames)
        ...
        plotEachOut = [plotOut filesep 'plot_sortScore_events' num2str(eventNumb) '_tolerance' num2str(timingTolerance) '_offset_' num2str(offPast) '_' num2str(offFuture) filesep testName filesep 'session' filesep sessionNumbers{testSubjID}];
        if ~isdir(plotEachOut)   % if the directory is not exist
            mkdir(plotEachOut);  % make the new directory
        end
        plot_sortScore(scoreData.combinedScore{1}, ...
