...
%
function [Yp, finalScore, finalCutoff, initProb, initCutoff] = ARRLS_imb(Xs, Xt, Ys, Yt, optionARRLS, optionIMB, optionETC)

    %if ~exist('.\liblinear\matlab', 'dir') 
    ...
    if optionIMB.BT == 1    % balance training samples
        [Xtmp, Ytmp] = balanceOverMinor_imb(sparse(X(:,1:Ns)), Y(1:Ns));
    else
    ...
    if optionIMB.AC1 == 0   % if adaptive cutoff flag is off
        initCutoff = 0.5;
    elseif optionIMB.AC1 == 1  % best cutoff
        Nt_n = sum(Yt == 0);
        trueLabel = [zeros(Nt_n, 1); ones(Nt-Nt_n, 1)];
        initCutoff = getCutoff_BEST(trueLabel, initProb, 'avrAcc', 100);
    elseif optionIMB.AC1 == 2  % fitting two Gaussians
        initCutoff = getCutoff_GMMs(initProb, 10, 1000, 0.01);
    elseif optionIMB.AC1 == 3 
        [initCutoff, mu1, sigma1, mu2, sigma2, xgrid] = getCutoff_FL1(initProb);
    elseif optionIMB.AC1 == 4 
        [initCutoff, mu1, sigma1, mu2, sigma2, xgrid] = getCutoff_FL2(initProb, 0.5);
...
%     finalCutoff = abs(bestCutoff-initCutoff);   % kyung
    ...
    Nt_p = sum(Yp==2);
    if optionETC.bVerbose == 1
        getResults_struct(Yp, Y(Ns+1:end), 'LR prediction');
    end
    ...
    finalScore = F(Ns+1:end, 2) - F(Ns+1:end, 1);

    if optionIMB.AC2 == 0   % if adaptive cutoff flag is off
        finalCutoff = 0.0;
    elseif optionIMB.AC2 == 1  % best cutoff
        Nt_n = sum(Yt == 0);
        trueLabel = [zeros(Nt_n, 1); ones(Nt-Nt_n, 1)];
        finalCutoff = getCutoff_BEST(trueLabel, finalScore, 'avrAcc', 100);
    elseif optionIMB.AC2 == 2  % fitting two Gaussians
        finalCutoff = getCutoff_GMMs(finalScore, 10, 1000, 0.01);
    elseif optionIMB.AC2 == 3 
        finalCutoff = getCutoff_FL1(finalScore);
    elseif optionIMB.AC2 == 4 
    ...
    Yp = (finalScore > finalCutoff) + 1;
    if optionETC.bVerbose == 1
        getResults_struct(Yp, Y(Ns+1:end), 'ARRLS prediction');
        fprintf('ARRLS terminated... accuracy, %f\n\n', numel(find(Yp==Y(Ns+1:end)))/Nt);
    end
end
