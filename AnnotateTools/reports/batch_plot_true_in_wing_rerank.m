...
%
function outPath = batch_plot_true_in_wing_rerank(inPath, inPath_rerank, varargin)
    ...
    fileList_rerank = dir([inPath_rerank filesep '*.mat']);
    
    if length(fileList) ~= length(fileList_rerank)
        error('file numbers are not matched');
    end
    
    % go over all test sets
    for testSubjID=1:length(fileList)
        ...
        annotData_rerank = load([inPath_rerank filesep fileList_rerank(testSubjID).name]); % load annotData
        positiveIndex = annotData_rerank.annotData.combinedScore > 0;
        negativeIndex = annotData_rerank.annotData.combinedScore <= 0;
		[~, sIdxPositive] = sort(annotData_rerank.annotData.combinedScore(positiveIndex), 'descend');
        sIdxNegative = sIdx(negativeIndex);
        
        sIdx = [sIdxPositive; sIdxNegative];
        ...
        hold on
        numbPositives = sum(positiveIndex);
