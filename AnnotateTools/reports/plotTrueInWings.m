function figh = plotTrueInWings(annotData, varargin)
...
%% Setup the parameters and reporting for the call   
    ...
    timingTolerance = 2;
    if isfield(params, 'timingTolerance')
    ...
    subwindowRange = 16;  %    2 seconds if subwindow is 125 ms
    if isfield(params, 'subwindowRange')
        subwindowRange = params.subwindowRange;
    end
    
    basethreshold = 0;  
    if isfield(params, 'basethreshold')
        basethreshold = params.basethreshold;
    end
    plotSize = 2*subwindowRange + 1; % Number subwindows in plot
    ...
    for n = 1:2:plotSize
        xTickLabel{n} = num2str(n - subwindowRange - 1);
    end
    ...
    centerIndex = find(annotData.wmScores > basethreshold);
    ...
    centerData = sortData(:, subwindowRange + 1);
    ...
    numbPositives = sum(annotData.wmScores > annotData.combinedCutoff);
    centerData = sortData(1:numbPositives, subwindowRange + 1);
    CRa = sum(centerData==1);
    IRa = sum(centerData==2);
    NRa = sum(centerData==3);
    CRb = sum(centerData==4);
    ...
    plotData = ones(length(centerIndex), plotSize)*8;     
        for i=1:length(centerIndex)
            ...
            if bIn
                ...
                plotData(i, subwindowRange + 1) = 7;     % no event
            end
        end
    end
    ...
    function newData = copyLabels(data, idx)
        ...
        srcStart = max(1, idx - subwindowRange);
        srcEnd = min(length(data), idx + subwindowRange);   
        destStart = max(1, subwindowRange + 2 - idx);
        destEnd = min(length(newData), subwindowRange + 1 + length(data) - idx);   
    ...
    % If a non-targetID value is within timingTolerance count as hit and mark 
        startIdx = max(1, curIdx - timingTolerance);
        endIdx = min(length(labels), curIdx + timingTolerance);
    ...
    %% See if there is a response subwindowOffset subwindows after stimulus event
        ...
        endIdx = min(length(labels), curIdx + subwindowRange);
        bIn = false;
        idx = curIdx;
