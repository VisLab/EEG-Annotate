function outPath = batch_classify_ARTLimb_reranking(inPath_test, inPath_train, inPath_annot, targetClass, varargin)
...
%
    ...
    fileList_annot = dir([inPath_annot filesep '*.mat']);
    if length(fileList_test) ~= length(fileList_annot)
        error('number of test sets and annotation sets are not same');
    end
    for i=1:length(fileList_test)
        testData = load([inPath_test filesep fileList_test(i).name]);
        annotData = load([inPath_annot filesep fileList_annot(i).name]);
        pseudoLabel = (annotData.annotData.combinedScore > 0);
        scoreData = classify_ARTLimbs(testData, inPath_train, targetClass, varargin{:}, 'pseudoLabel', pseudoLabel);
