function [] = batchPositiveSamples(inPath, outPath, classLabel, ...
...
for k = 1:length(fileList)
    ...
    fileName = sprintf('%s_positive_%s.mat', theName, classLabel);
    save([outPath filesep fileName], 'classLabel', 'tolerance', ...
            'samples', 'labels', 'timeTolerance', ...
            'sampleIndex', 'nearestEvent', '-v7.3')
    if exclude
        ...
        for j = 1:length(scoreData)
            ...
            excludeScoreData = thisScoreData(j);
            thisScoreData(j) = [];
            [labels, samples, sampleIndex, timeTolerance, nearestEvent] = ...
                getPositiveSamples(thisScoreData, classLabel, tolerance, varargin);  %#ok<NASGU,ASGLU>
            [~, thisName, ~] = fileparts(excludeScoreData.trainFileName);
            fprintf('    %s\n', thisName);
            fileName = sprintf('%s_ex_%s_positive_%s.mat', ...
                theName, thisName, classLabel);
            save([outPath filesep fileName], 'classLabel', 'tolerance', ...
                'samples', 'labels', 'timeTolerance', ...
                'sampleIndex', 'nearestEvent', '-v7.3')
        end
